		...
		for (Component comp : this.todosComponentes) {
			...
			if (comp instanceof JScrollPane) {
				...
				for (int t = 0; t < components.length; ++t) {
					if (!(components[t] instanceof JTextArea)) {
						continue;
					}
					JTextArea textArea = (JTextArea) components[t];
					textArea.setPreferredSize(new Dimension(740, 25));
					textArea.setMinimumSize(new Dimension(740, 25));
					textArea.setSize(new Dimension(740, 25));
					textArea.setMaximumSize(new Dimension(740, 25));
				}
			}
		}
	}
