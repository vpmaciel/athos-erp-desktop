...
public final class ClientePC extends JPanel implements GUI {
	...
	public void iniciarGUI() {
		...
		Collections.sort(bancos, new BancoCPT().new Nome());
		for (Banco b : bancos) {
			boxBanco.addItem(b);
		}
	...
	public void reiniciarGUI() {
		...
		if (!MainCT.getClienteFC().isShowing()
				&& MainCT.getClienteFC().getClienteHandle().getCliente() != null) {
			empresa = MainCT.getClienteFC().getClienteHandle().getCliente().getEmpresa();
			if (empresa != null) {
				boxEmpresa.setSelectedItem(empresa);
			}
			banco = MainCT.getClienteFC().getClienteHandle().getCliente().getBanco();
