...
final class CaracteristicaImp implements CaracteristicaDao {
	...
	public Collection<Caracteristica> getRegistro() {
		...
		try {
			...
			entityTransaction = entityManager.getTransaction();
			entityTransaction.begin();
			Query query = entityManager.createQuery("select T from Caracteristica T order by T.funcionario",
	...
	public Collection<Caracteristica> pesquisarRegistro(Caracteristica caracteristica) {
		...
		try {
			...
			if (caracteristica.getFuncionario() != null && caracteristica.getFuncionario().getId() != null) {
				predicateList.add(
						criteriaBuilder.equal(rootCaracteristica.get("funcionario"), caracteristica.getFuncionario()));
			}
			if (caracteristica.getAdequado() != null && caracteristica.getAdequado().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("adequado"),
						"%" + caracteristica.getAdequado() + "%"));
			}
			if (caracteristica.getAgil() != null && caracteristica.getAgil().length() > 0) {
				predicateList.add(
						criteriaBuilder.like(rootCaracteristica.get("agil"), "%" + caracteristica.getAgil() + "%"));
			}
			if (caracteristica.getAgitado() != null && caracteristica.getAgitado().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("agitado"),
						"%" + caracteristica.getAgitado() + "%"));
			}
			if (caracteristica.getAlegre() != null && caracteristica.getAlegre().length() > 0) {
				predicateList.add(
						criteriaBuilder.like(rootCaracteristica.get("alegre"), "%" + caracteristica.getAlegre() + "%"));
			}
			if (caracteristica.getAmavel() != null && caracteristica.getAmavel().length() > 0) {
				predicateList.add(
						criteriaBuilder.like(rootCaracteristica.get("amavel"), "%" + caracteristica.getAmavel() + "%"));
			}
			if (caracteristica.getAnalitico() != null && caracteristica.getAnalitico().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("analitico"),
						"%" + caracteristica.getAnalitico() + "%"));
			}
			if (caracteristica.getAnimado() != null && caracteristica.getAnimado().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("animado"),
						"%" + caracteristica.getAnimado() + "%"));
			}
			if (caracteristica.getAnsioso() != null && caracteristica.getAnsioso().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("ansioso"),
						"%" + caracteristica.getAnsioso() + "%"));
			}
			if (caracteristica.getApatico() != null && caracteristica.getApatico().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("apatico"),
						"%" + caracteristica.getApatico() + "%"));
			}
			if (caracteristica.getArticulado() != null && caracteristica.getArticulado().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("articulado"),
						"%" + caracteristica.getArticulado() + "%"));
			}
			...
			if (caracteristica.getAtencioso() != null && caracteristica.getAtencioso().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("atencioso"),
						"%" + caracteristica.getAtencioso() + "%"));
			}
			if (caracteristica.getAtivo() != null && caracteristica.getAtivo().length() > 0) {
				predicateList.add(
						criteriaBuilder.like(rootCaracteristica.get("ativo"), "%" + caracteristica.getAtivo() + "%"));
			}
			...
			if (caracteristica.getAventureiro() != null && caracteristica.getAventureiro().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("aventureiro"),
						"%" + caracteristica.getAventureiro() + "%"));
			}
			if (caracteristica.getBemHumorado() != null && caracteristica.getBemHumorado().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("bemHumorado"),
						"%" + caracteristica.getBemHumorado() + "%"));
			}
			if (caracteristica.getCalmo() != null && caracteristica.getCalmo().length() > 0) {
				predicateList.add(
						criteriaBuilder.like(rootCaracteristica.get("calmo"), "%" + caracteristica.getCalmo() + "%"));
			}
			if (caracteristica.getCarismatico() != null && caracteristica.getCarismatico().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("carismatico"),
						"%" + caracteristica.getCarismatico() + "%"));
			}
			if (caracteristica.getCauteloso() != null && caracteristica.getCauteloso().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("cauteloso"),
						"%" + caracteristica.getCauteloso() + "%"));
			}
			if (caracteristica.getCompetitivo() != null && caracteristica.getCompetitivo().length() > 0) {
				predicateList.add(criteriaBuilder.like(rootCaracteristica.get("competitivo"),
						"%" + caracteristica.getCompetitivo() + "%"));
			}
